...
five.Board().on('ready', function() {
  ...
  const tilt = new five.Button(2); // digital pin 2

 

  // tilt the breadboard to the right, towards to the ground pin
  tilt.on("down", function() {valorMapeado2 = -1});

  // tilt and hold
  tilt.on("hold", function() {valorMapeado2 = 0});

  // tilt back the breadboard to the stable position
  tilt.on("up", function() {valorMapeado2 = 1});
  ...
  sensor.on("data", function() {
    ...
    valorMapeado = five.Fn.constrain(five.Fn.map(valorSensor, 9, 500, 0, 100),0,1000); //sound
    //valorMapeado2 = five.Fn.constrain(five.Fn.map(valorSensor, 9, 90, 0, 45),-45,45); //tilt
...
io.on('connection', function(socket){
    ...
    setInterval(() => {
      ...
      socket.emit('envioValorSensorMapeado', valorMapeado, valorMapeado2); //con emit envío un valor
    }, 100)
